...
$pedido=$respuesta['parametros']['entidadRespuesta']['descripcion']['producto'];

$visto=$respuesta['parametros']['entidadRespuesta']['visto_produccion'];
$rol=$respuesta['rol'];
$cod_pedido=$respuesta['parametros']['entidadRespuesta']['codpedido'];
...
<?php
if ($respuesta['editar']== FALSE && $accion!='n'){
          ...
          <tr>
            <th>
              <div>
                <?php
                                   //dump($rubro['cantidad']);
                  $pgSql = DB::table('tipo_rubro')
                                            ->select ('nombre')
                                            ->where  ('cod_tipo','=',$rubro['cod_tipo_rubro'])
                                            ->get();
                   $pgSql=  $pgSql['0']->nombre;
                ?>
                   {{$pgSql}}
              </div>
            </th>
            ...
            <th>
              <div>
                <?php
                                   //dump($rubro['cod_rubro']);
                  $pgSql = DB::table('rubro')
                                            ->select ('nombre')
                                            ->where  ('cod_rubro','=',$rubro['cod_producto'])
                                            ->get();
                  $pgSql=  $pgSql['0']->nombre;
                  ?>
                  {{$pgSql}}
...
if ($respuesta['editar']== TRUE && $accion!='n'){
//dump($respuesta)
?>
        @foreach($pedido as $key => $rubro)
        <tr id="fila-{{$key}}">
          <th>
            ...
            <select class='input form-control' id='cod_tipo_rubro_{{$key}}' name='cod_tipo_rubro[]' onchange='llenar_rubros({{$key}});'>
                  ...
                  $pgSql = DB::table('tipo_rubro')
                                          ...
                                          echo '<option value="0" disabled="disabled">Selecione Fabricante </option>';
                                          foreach ($pgSql as $key => $clave) 
                                          {
                                            $selected=null;
          ...
          <th>
            <div>
            <select class='input form-control cod_producto' id='cod_producto_{{$key}}' name='cod_producto[]' onchange='select_producto({{$key}});'>   
                <?php
                                   //dump($rubro['cantidad']);
                  $pgSql = DB::table('rubro')
...
//dd($respuesta);
if ($respuesta['editar'] ==TRUE || $rol=='RL03')  {
?>          
          <div class="col-md-offset-5 col-xs-offset-4">
          <button type="button" value="" id="confirmar_pedido" class="btn btn-primary"  onclick="modal_operacion();">OPERACION A REALIZAR</button>
          </div>
<?php
}
?>
...
<div  id="modal-confirmar" class="modal fade bs-modal-confirmar-modal-sm" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
  <div class="modal-dialog modal-sm" role="document">
    <div class="modal-content">

     <h4 class="text-center">CONFIRMA EL PEDIDO DENTRO DEL SISTEMA?</h4>
...
<div  id="modal-operaqcion" class="modal fade bs-modal-operaqcion" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
     <h4 class="text-center">QUE OPERACION DESEA REALIZAR RESPECTO AL PEDIDO</h4>

          <div class="form-group" style="margin-left: 30px; width: 85%;">



            <label for="comment">Seleccione Una Opcion:</label>
            <select class='input form-control' id='cod_estatus' name='cod_estatus' onchange='operaciones()'>
                    <?php
                                     //dump($rubro['cantidad']);
  
                                            echo '<option value="0">Selecione Fabricante </option>';
                                            foreach ($respuesta['listar_estatus'] as $key => $estatus) 
                                            {           
                                              var_export('<option value="'.$estatus['cod_tipo_estatus'].'" >'.$estatus['nombre_seleccion'].'</option>');
                                            }
                    ?>
              </select>
     ...
     <div class="modal-footer">
     <h5 class="text-center"> EL PEDIDO NO SE PODRA RECUPERAR ¿DESEA CONTINUAR CON LA OPERACION?</h5>
        <div class="text-center">
        <button type="submit" id="refrescar" class="btn btn-success">SI</button>
        <button type="button" class="btn btn-danger"  data-dismiss="modal">NO</button> 
        </div>
...
function modal_operacion()
{
$('#modal-operaqcion').modal('show')

  //$('#cod_estatus').val()=$('#cancelar_pedido').val()
}
...
{
  alert($('#cod_estatus').val())

  if ($('#cod_estatus').val()==3 || $('#cod_estatus').val()==7) 
  {
    $('#div_observacion').show();

  }

  if ($('#cod_estatus').val()==2) 
  {
